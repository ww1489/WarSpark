...
本文档基于 `docs/product/content-data-policy.md` 和 `docs/architecture/data-model.md`，用于约束后续内容导入、人工审核、链接检查和运营流程实现。
...
| 子模块 | 职责 |
| --- | --- |
| Source Intake | 接收公开页面、授权导入、人工录入和找阵结果沉淀。 |
| Source Normalization | 标准化图片、链接、视频、TH、标签和来源字段。 |
| Review Workflow | 维护审核状态、质量状态和公开可见性。 |
| Link Checker | 检查 OpenLayout 和 YouTube 链接可用性。 |
...
| 状态 | 使用场景 |
| --- | --- |
| `high_confidence` | 人工确认、来源完整、图片和链接可用。 |
...
| OpenLayout 格式错误 | 标记 `broken` 或拒绝链接。 |
...
- 公开来源、授权导入、人工录入和找阵沉淀的路径清晰。
- 每类内容都有来源记录、审核状态、质量状态或链接状态。
- 用户上传图片不会未经审核直接公开。
- OpenLayout 和 YouTube 只保存引用和状态，不绕过访问限制。
