...
战争数据是 WarSpark 从“截图找阵工具”升级为“战争情报站”的关键能力，但它不作为 MVP 找阵流程的硬依赖。用户即使没有输入部落 tag 或玩家 tag，也必须能够独立完成截图找阵。
...
MVP 只保留页面入口和数据模型预留；V1 再接入完整战争情报页。
...
MVP 可以使用 `image_search_jobs.target_context` JSON 保存目标上下文；V1 可使用 `war_targets` 独立表。
