...
func TestLeagueServiceGetLeagueSeasons(t *testing.T) {
	...
	if err != nil { t.Fatalf("error: %v", err) }
	if len(resp.Items) != 1 || resp.Items[0].ID != "2026-01" { t.Fatalf("got %+v", resp.Items) }
}
...
func TestLeagueServiceGetLeagueSeasonRankings(t *testing.T) {
	...
	if err != nil { t.Fatalf("error: %v", err) }
	if len(resp.Items) != 1 || resp.Items[0].Name != "TopPlayer" { t.Fatalf("got %+v", resp.Items) }
}
...
func TestLeagueServiceGetLeagueTiers(t *testing.T) {
	...
	if err != nil { t.Fatalf("error: %v", err) }
	if len(resp.Items) != 1 || resp.Items[0].Name != "Champion I" { t.Fatalf("got %+v", resp.Items) }
}
...
func TestLeagueServiceGetLeagueTier(t *testing.T) {
	...
	if err != nil { t.Fatalf("error: %v", err) }
	if lt.Name != "Champion I" { t.Fatalf("got %q", lt.Name) }
}
...
func TestLeagueServiceGetWarLeague(t *testing.T) {
	...
	if err != nil { t.Fatalf("error: %v", err) }
	if wl.Name != "Master" { t.Fatalf("got %q", wl.Name) }
}
