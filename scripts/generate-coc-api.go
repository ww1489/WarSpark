...
// cocapi 代码生成器:从官方 Swagger 2.0 规范生成 spec.yaml / types.go / api.go。
//
// 权威来源:pkg/cocapi/official-swagger.yaml(从 https://developer.clashofclans.com 下载)
// 用法:go run scripts/generate-coc-api.go
//
// 工作流(API 变动时):
//  1. 重新从官方文档下载 swagger.yaml,替换 pkg/cocapi/official-swagger.yaml
//  2. 运行本脚本
//  3. gofmt -w pkg/cocapi/ && go test ./pkg/cocapi/...
package main
...
func main() {
	...
	specPath := filepath.Join(root, "pkg", "cocapi", "official-swagger.yaml")
	...
	endpoints := collectEndpoints(&sw)
	defs := orderDefinitions(&sw)

	genSpecYAML(filepath.Join(dir, "spec.yaml"), endpoints)
	genTypesGo(filepath.Join(dir, "types.go"), &sw, defs)
	genAPIGo(filepath.Join(dir, "api.go"), endpoints)

	fmt.Printf("生成完成: %d 个端点, %d 个定义\n", len(endpoints), len(defs))
}
...
func genSpecYAML(path string, eps []GenEndpoint) {
	var b strings.Builder
...
func genTypesGo(path string, sw *Swagger, defs []string) {
	var b strings.Builder
...
func genAPIGo(path string, eps []GenEndpoint) {
	var b strings.Builder
	b.WriteString("// Code generated by scripts/generate-coc-api.go from official-swagger.yaml; DO NOT EDIT.\n")
...
func fail(format string, args ...any) {
	fmt.Fprintf(os.Stderr, "generate-coc-api: "+format+"\n", args...)
	os.Exit(1)
}
